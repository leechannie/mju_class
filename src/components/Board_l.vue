...
<script>
//import data from '@/data/lost.js' //data 파일에서 데이터 불러오기
...
export default {
  ...
  methods: {
    rowClick(item) {
      this.$router.push({
        path: `/board_l/free/detail/${item.content_id}`
      })
    },
    writeContent(){
      this.$router.push({
        path: '/board_l/free/create'
      })
    },
    async read(){
      var db = firebase.firestore();
      const sn = await db.collection("board_l").orderBy("content_id","desc").get()
